...
use App\Services\FishingSyncService;
use Illuminate\Console\Command;
use App\Services\XivApiService;
use Throwable;

class SyncFishing extends Command
{
    ...
    protected $signature = 'fishing:sync-range
    {job}
    {minLevel}
    {maxLevel}';

    protected $description =
    'Sync fishing data for all raw materials in a crafting level range';



    public function handle(
        XivApiService $xivApiService,
        FishingSyncService $fishingSyncService,
    ): int {
        ...
        $this->info(
            'Found ' . count($materials) . ' raw material(s).'
        );
        ...
        foreach ($materials as $material) {
            ...
            if (!$fishingSyncService->needsSync($material['id'])) {
                $stats['fresh']++;
            ...
            try {
                $fishingSyncService->sync($material['id']);
        ...
        $this->info('Fishing sync complete.');
